    ...
    for item in _pubsub.listen():
        ...
        if item['channel'] == 'bmp280':
            ...
            if t_start is None:
                ...
                t_stop = _data['p_utc']
                break
    print('### measured {} samples in {:.2f}s ###'.format(len(pressure_history),
        t_stop-t_start))
    print('p_mean = {:.2f} +/- {:.2f} Pa'.format(np.mean(pressure_history),
        np.std(pressure_history)))
    print('t_mean = {:.2f} +/- {:.2f} °C'.format(np.mean(temperature_history),
        np.std(temperature_history)))
